...
class GFFormsModel {
	...
	public static function get_file_upload_path( $form_id, $file_name ) {
		...
		if ( ! is_dir( $target_root ) ) {
			...
			if ( $default_target_root != $target_root && ! file_exists( $target_root . 'index.html-head' ) ) {
				GFCommon::recursive_add_index_file( $target_root );
			} elseif ( ! file_exists( self::get_upload_root() . '/index.html-head' ) ) {
				GFCommon::recursive_add_index_file( self::get_upload_root() );
			} elseif ( ! file_exists( self::get_upload_path( $form_id ) . '/index.html-head' ) ) {
				GFCommon::recursive_add_index_file( self::get_upload_path( $form_id ) );
			} elseif ( ! file_exists( self::get_upload_path( $form_id ) . "/$y/index.html-head" ) ) {
