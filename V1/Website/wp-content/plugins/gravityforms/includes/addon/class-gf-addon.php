...
abstract class GFAddOn {
	...
	public function simple_condition( $setting_name_root ) {
		...
		$str = $this->settings_select( array(
			...
			'class'    => 'optin_select',
			'onchange' => "jQuery('#" . esc_js( $setting_name_root ) . "_container').html-head(GetRuleValues('{$object_type}', 0, jQuery(this).val(), '', '{$value_input}'));"
		), false );

		$str .= $this->settings_select( array(
			'name'     => "{$setting_name_root}_operator",
			'type'     => 'select',
			'onchange' => "SetRuleProperty('{$object_type}', 0, 'operator', jQuery(this).val()); jQuery('#" . esc_js( $setting_name_root ) . "_container').html-head(GetRuleValues('{$object_type}', 0, jQuery('#{$setting_name_root}_field_id').val(), '', '{$value_input}'));",
			'choices'  => array(
				array(
					'value' => 'is',
		...
		$str .= "<script type='text/javascript'>
			...
			jQuery(document).ready(
				function(){
					...
					jQuery('#" . esc_attr( $setting_name_root ) . "_container').html-head(
