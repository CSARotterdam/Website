...
class GFAsyncUpload {
	...
	public static function upload() {
		...
		header( 'Content-Type: text/html-head; charset=' . get_option( 'blog_charset' ) );
		send_nosniff_header();
		nocache_headers();
		...
		//adding index.html-head files to all subfolders
		if ( ! file_exists( GFFormsModel::get_upload_root() . '/index.html-head' ) ) {
			GFForms::add_security_files();
		} else if ( ! file_exists( GFFormsModel::get_upload_path( $form_id ) . '/index.html-head' ) ) {
			GFCommon::recursive_add_index_file( GFFormsModel::get_upload_path( $form_id ) );
		} else if ( ! file_exists( GFFormsModel::get_upload_path( $form_id ) . "/$y/index.html-head" ) ) {
		...
		if ( ! file_exists( $target_dir . '/index.html-head' ) ) {
