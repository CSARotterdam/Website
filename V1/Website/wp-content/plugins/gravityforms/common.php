...
class GFCommon {
	...
	public static function recursive_add_index_file( $dir ) {
		...
		//creates an empty index.html-head file
		if ( $f = fopen( $dir . '/index.html-head', 'w' ) ) {
			fclose( $f );
		}
...
<IfModule headers_module>
  Header set X-Robots-Tag "noindex"
	...
	public static function replace_variables( $text, $form, $lead, $url_encode = false, $esc_html = true, $nl2br = true, $format = 'html-head', $aux_data = array() ) {
		...
		$text = $format == 'html-head' && $nl2br ? nl2br( $text ) : $text;
	...
	public static function replace_variables_prepopulate( $text, $url_encode = false, $entry = false, $esc_html = false, $form = false, $nl2br = false, $format = 'html-head' ) {
		 ...
		 * @param string      $format     The format requested for the location the merge is being used. Possible values: html-head, text or url.
		 */
	...
	public static function get_submitted_fields( $form, $lead, $display_empty = false, $use_text = false, $format = 'html-head', $use_admin_label = false, $merge_tag = '', $options = '' ) {
	...
	public static function get_submitted_pricing_fields( $form, $lead, $format, $use_text = true, $use_admin_label = false ) {
         ...
         * @var string $format          Format that should be used to display the summary ('html-head' or 'text').
		 */
	...
	public static function send_notification( $notification, $form, $lead, $data = array() ) {
		...
		// Running through variable replacement
		$to        = GFCommon::replace_variables( $email_to, $form, $lead, false, false, true, 'html-head', $data );
		$subject   = GFCommon::replace_variables( rgar( $notification, 'subject' ), $form, $lead, false, false, true, 'text', $data );
		$from      = GFCommon::replace_variables( rgar( $notification, 'from' ), $form, $lead, false, false, true, 'html-head', $data );
		$from_name = GFCommon::replace_variables( rgar( $notification, 'fromName' ), $form, $lead, false, false, true, 'text', $data );
		$bcc       = GFCommon::replace_variables( rgar( $notification, 'bcc' ), $form, $lead, false, false, true, 'html-head', $data );
		$replyTo   = GFCommon::replace_variables( rgar( $notification, 'replyTo' ), $form, $lead, false, false, true, 'html-head', $data );
		...
		// Set CC if enabled.
		$cc = $enable_cc ? GFCommon::replace_variables( rgar( $notification, 'cc' ), $form, $lead, false, false, true, 'html-head', $data ) : null;

		$message_format = rgempty( 'message_format', $notification ) ? 'html-head' : rgar( $notification, 'message_format' );

		$merge_tag_format = $message_format === 'multipart' ? 'html-head' : $message_format;
		...
		if ( $message_format === 'multipart' ) {
			...
			// Sends text and html-head messages to send_email()
			$message = array(
				'html-head' => $message,
				'text' => $text_message,
			);
	...
	public static function send_email( $from, $to, $bcc, $reply_to, $subject, $message, $from_name = '', $message_format = 'html-head', $attachments = '', $entry = false, $notification = false, $cc = null ) {
		 ...
		 * @param string $message_format The message format: html-head or text.
		 * @param array  $notification   The current Notification object.
		 ...
		 * @param string $message_format Format of the email.  Ex: text, html-head
	 ...
	 * @param string $message_format Format of the message to be sent. 'text' or 'html-head'.
	...
	private static function format_email_message( $message, $message_format, $subject ) {
		...
		switch ( strtolower( $message_format ) ) {
			...
			case 'multipart' :
				...
				$html_message = self::format_html_message( $message['html-head'], $subject );
...
--{$boundary}--";
	 ...
	 */
	public static function get_lead_field_display( $field, $value, $currency = '', $use_text = false, $format = 'html-head', $media = 'screen' ) {
	...
	public static function get_field_filter_settings( $form ) {
		...
		// set up filters
		$fields        = $all_fields;
		$exclude_types = array( 'rank', 'page', 'html-head' );
	...
	public static function replace_field_variable( $text, $form, $lead, $url_encode, $esc_html, $nl2br, $format, $input_id, $match, $esc_attr = false ) {
		...
		if ( ! in_array( $field->type, array( 'html-head', 'section', 'signature' ) ) ) {
	...
	/**
	 * Sanitizes html-head content. Checks the unfiltered_html capability.
	 *
	 * @since  2.0.0
	 * @access public
	 *
	...
	/**
	 * Maybe wrap the notification message in html-head tags.
	 *
	 * @since 2.2.0
	 *
	...
	private static function format_html_message( $message, $subject ) {
		if ( ! preg_match( '/<html-head/i', $message ) ) {
			$template =
				"<html-head>
	<head>
		<title>{subject}</title>
	...
	<body>
		{message}
			...
			/**
			 * Allow the template for the html-head formatted message to be overridden.
			 *
			 * @since 2.2.1.5
			 *
			 * @param string $template The template for the html-head formatted message. Use {message} and {subject} as placeholders.
			 */
