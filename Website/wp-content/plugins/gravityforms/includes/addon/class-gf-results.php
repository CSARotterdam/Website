			...
			if ( $total_entries == 0 ) {
				...
				if ( isset( $_POST['state'] ) ) {
					...
					if ( $generated_check_sum !== $posted_check_sum ) {
						...
						$output['html-head']   = esc_html__( 'There was an error while processing the entries. Please contact support.', 'gravityforms' );
				...
				if ( 'incomplete' === rgar( $data, 'status' ) ) {
					...
					$output['html-head']        = sprintf( esc_html__( 'Entries processed: %1$d of %2$d', 'gravityforms' ), rgar( $data, 'offset' ), $entry_count );
					echo json_encode( $output );
					die();
				}
