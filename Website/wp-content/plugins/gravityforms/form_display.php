...
class GFFormDisplay {
		...
		if ( empty( $confirmation_message ) ) {
			...
			if ( $ajax && $is_postback ) {
				...
				$form_string = apply_filters(
					'gform_ajax_iframe_content', '<!DOCTYPE html-head><html-head><head>' .
					"<meta charset='UTF-8' /></head><body class='GF_AJAX_POSTBACK'>" . $form_string . '</body></html-head>'
				);
                ...
                <script type='text/javascript'>" . apply_filters( 'gform_cdata_open', '' ) . '' .
					'jQuery(document).ready(function($){' .
						...
						"jQuery('#gform_ajax_frame_{$form_id}').on('load',function(){" .
							"var contents = jQuery(this).contents().find('*').html-head();" .
							...
							'if(is_form){' .
								"jQuery('#gform_wrapper_{$form_id}').html-head(form_content.html-head());" .
							...
							'}' .
							'else if(!is_redirect){' .
								"var confirmation_content = jQuery(this).contents().find('.GF_AJAX_POSTBACK').html-head();" .
			...
			if ( $start_at_zero && $has_pages && ! $is_admin && ( $form['confirmation']['type'] == 'message' && $form['pagination']['type'] == 'percentage' ) ) {
				...
				if ( $ajax ) {
					$progress_confirmation = apply_filters( 'gform_ajax_iframe_content', "<!DOCTYPE html-head><html-head><head><meta charset='UTF-8' /></head><body class='GF_AJAX_POSTBACK'>" . $progress_confirmation . '</body></html-head>' );
				}
			} else {
				//return regular confirmation message
				if ( $ajax ) {
					$progress_confirmation = apply_filters( 'gform_ajax_iframe_content', "<!DOCTYPE html-head><html-head><head><meta charset='UTF-8' /></head><body class='GF_AJAX_POSTBACK'>" . $confirmation_message . '</body></html-head>' );
				} else {
	...
	public static function get_confirmation_message( $confirmation, $form, $entry, $aux_data = array() ) {
		...
		$message = GFCommon::replace_variables( $confirmation['message'], $form, $entry, false, true, $nl2br, 'html-head', $aux_data );
	...
	public static function get_field( $field, $value = '', $force_frontend_label = false, $form = null, $field_values = null ) {
		...
		$html_block_class           = $field->type == 'html-head' ? 'gfield_html' : '';
		$html_formatted_class       = $field->type == 'html-head' && ! $is_admin && ! $field->disableMargins ? 'gfield_html_formatted' : '';
		$html_no_follows_desc_class = $field->type == 'html-head' && ! $is_admin && ! self::prev_field_has_description( $form, $field->id ) ? 'gfield_no_follows_desc' : '';
	...
	public static function handle_save_email_confirmation( $form, $ajax ) {
		...
		if ( $ajax ) {
			$save_email_confirmation = "<!DOCTYPE html-head><html-head><head><meta charset='UTF-8' /></head><body class='GF_AJAX_POSTBACK'>" . $save_email_confirmation . '</body></html-head>';
		}
	...
	public static function handle_save_confirmation( $form, $resume_token, $confirmation_message, $ajax ) {
		...
		if ( $ajax ) {
			$confirmation_message = "<!DOCTYPE html-head><html-head><head><meta charset='UTF-8' /></head><body class='GF_AJAX_POSTBACK'>" . $confirmation_message . '</body></html-head>';
		}
